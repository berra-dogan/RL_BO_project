...
from experiments.run_one_reward_experiments import BASE_SETTINGS
...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Path under the repository root, or an absolute path, to a best_config.json. Can be repeated.",
    )
    parser.add_argument(
        "--reward",
        nargs="+",
...
def apply_settings(payload, device, num_runs_override):
    ...
    base.update(payload.get("base_settings", {}))
