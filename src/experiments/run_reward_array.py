...
def base_settings_for_reward(reward_name, config):
    settings = dict(BASE_SETTINGS)
    settings["reward_mode"] = reward_name
    if "snake_path_cost_weight" in config:
        settings["snake_path_cost_weight"] = config["snake_path_cost_weight"]
...
def run_array_job(args):
    ...
    if args.skip_existing and result_path.exists():
        if config_path.exists() and json.loads(config_path.read_text()) == expected_config:
            ...
            return
        raise RuntimeError(
            "Existing result does not match the current reward grid. "
...
def collect_reward(output_root, reward_name):
    ...
    best_payload = {
        ...
        "base_settings": base_settings_for_reward(reward_name, best_row),
        "reward": reward_name,
